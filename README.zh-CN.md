...
这是**一个本地 stdio MCP Server**，不是浏览器扩展，也不是四个独立 MCP。它管理由 Jev 创建的 Chrome 页面，并暴露四个供模型调用的工具：
...
服务会在打开浏览器前拒绝交易型目标；在动作执行前拦截购买、结算、付款、上传、密码、银行卡和验证码控件。上游的 DOM 快照本身也会排除密码和文件控件。因此这是浏览/采集工具，而不是代购或下单机器人。
